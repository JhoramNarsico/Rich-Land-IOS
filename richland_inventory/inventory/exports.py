...
def add_word_footer(document):
    """Adds a standard footer to the bottom of the Word document."""
    document.add_paragraph()
    footer_p = document.add_paragraph()
    footer_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    footer_run = footer_p.add_run(f"Generated by Rich Land Inventory System on {timezone.now().strftime('%B %d, %Y %I:%M %p')}")
    footer_run.font.name = 'Arial'
    footer_run.font.size = Pt(8)
    footer_run.italic = True
    footer_run.font.color.rgb = RGBColor(0x95, 0xA5, 0xA6)
...
def generate_sow_history_export(customer, sows, format_type, request):
    ...
    if format_type == 'excel':
        ...
        for sow in sows:
            ...
            for col in range(1, 9): ws.cell(row=current_row, column=col).border = thin_border
        ...
        run = p.add_run(f"Total Service Cost: PHP {total_cost:,.2f}")
        run.bold = True; run.font.name = 'Arial'; run.font.size = Pt(12)
        
        add_word_footer(document)
...
def generate_expense_report(expenses, format_type, request):
    ...
    if format_type == 'excel':
        ...
        for expense in expenses:
            ...
            for col in range(1, 6): ws.cell(row=current_row, column=col).border = thin_border
        ...
        for expense in expenses:
            ...
            alignments =[WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.CENTER, WD_ALIGN_PARAGRAPH.RIGHT]
            ...
            for i, cell in enumerate(row_cells):
                ...
                p.paragraph_format.space_before = Pt(3)
                p.paragraph_format.space_after = Pt(3)
                for run in p.runs: run.font.name = 'Arial'; run.font.size = Pt(9)
        ...
        for customer in customers:
            ...
            for i, cell in enumerate(row_cells):
                ...
                for p in cell.paragraphs:
                    p.paragraph_format.space_before = Pt(3)
                    p.paragraph_format.space_after = Pt(3)
                    if i == 3: p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                    for run in p.runs: run.font.name = 'Arial'; run.font.size = Pt(9)
...
def generate_customer_statement(customer, final_data, running_balance, format_type, request):
    ...
    if format_type == 'excel':
        ...
        p = document.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.paragraph_format.space_before = Pt(12)
        run = p.add_run(f"Total Outstanding Balance: PHP {running_balance:,.2f}")
...
def generate_supplier_deliveries_export(supplier, purchase_orders, format_type, request):
    ...
    if format_type == 'excel':
        ...
        for po in purchase_orders:
            ...
            if not po.items.exists():
                ...
                for item in po.items.all():
                    ...
                    c4 = ws.cell(row=current_row, column=4, value=item.price); c4.number_format = '#,##0.00'; c4.border = thin_border; c4.alignment = Alignment(horizontal='right')
                    c5 = ws.cell(row=current_row, column=5, value=item.line_total); c5.number_format = '#,##0.00'; c5.font = Font(bold=True); c5.border = thin_border; c5.alignment = Alignment(horizontal='right')
        ...
        for po in purchase_orders:
            ...
            if not po.items.exists():
                row_cells = table.add_row().cells; row_cells[0].merge(row_cells[4]); row_cells[0].text = "No items found."; row_cells[0].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
            else:
                for item in po.items.all():
                    row_cells = table.add_row().cells
                    ...
                    alignments =[WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.CENTER, WD_ALIGN_PARAGRAPH.RIGHT, WD_ALIGN_PARAGRAPH.RIGHT]
                    for i, cell in enumerate(row_cells):
                        cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
